...
"""This module holds the maze creation and path finder methods"""


class Cell:
    """This class defines the cells of a maze"""
    ...
    def open_path(self, other: "Cell", wall: str) -> None:
        """This method opens both walls for the current and the given cell

        Parameters:
        other (Cell): neighbour of this current cell
        wall (str): The wall we would like to open
        """
...
class Maze:
    """This class defines a maze that is a combination of cell classes"""
    ...
    def cell_at(self, x: int, y: int) -> Cell:
        ...
        Returns:
        Cell: The cell at given coordinates
        """
        return self.maze_map[x][y]

    def get_unvisited_neighbors(self, cell: Cell) -> list[tuple[str, Cell]]:
        """Gets all the unvisited neighbors for a cell

        Parameters:
        cell: The cell that we want to get its neighbors

        Returns:
        list: list of unvisited neighbors
        """
        neighbors = []
    ...
    def embed_42(self) -> None:
        """This function embeds the 42 logo to the middle of the maze"""
        logo_cells = set()
        cx = (self.nx // 2) - 3
        cy = (self.ny // 2) - 2
    ...
    def seal_logo_borders(self) -> None:
        """This function is a check that seals the 42 logo borders"""
    ...
    def has_large_open_area(self, x: int, y: int) -> bool:
        """This function checks if there are any 2x3 or 3x2 open areas
        ...
        Returns:
        bool: returns true if there is a large open area
        """
        for dx in range(-1, 2):
            for dy in range(-1, 2):
                bx, by = x + dx, y + dy
    ...
    def make_imperfect(self) -> None:
        """This function breaks extra walls without creating large open areas
        to make the maze imperfect"""
        walls = []
    ...
    def generate_maze(self, config: dict[str, Any]) -> None:
        """This is the first maze generation algorithm,

        It works by selecting
        a random unvisited neighbor the current cell has and goes there by
        breaking a wall and continues doing this untill there are no unvisited
        neighbors

        Parameters:
        config: The formatted version of config.txt
        """
        self.logo_cells = set()
    ...
    def Loop_erased_random_walk(self, config: dict[str, Any]) -> None:
        """This fucntion governs the second maze gen algorithm

        In this Algorithm a list of valid cells is created and from that
        a random cell is selected, this cell is then marked as being in maze
        a second cell is then selected and from this cell a random path is
        generated by continually selecting one of the neighbours of this cell
        at random, taking care to not pick the parent cell of our current cell
        all these cells are added to a path, when this path loops back unto
        itself the algorithm cuts the loop from the path and the walk continues

        once this path meets up with a cell that is markes as in maze the path
        gets added to the maze and a random cell gets picked to starts this
        process again

        this loops untill there are no more valid cells to choose as starting
        position
        """
        if config["seed"] is not None:
            random.seed(config["seed"])
    ...
    def build_path(self, current_path: list[Cell]) -> None:
        """This function loops through the path builind it out"""
    ...
    def prune_valid(self, valid_cells: set[Cell]) -> set[Cell]:
        """This function prunes the set of valid cells

        it achieves this by checking for all cells in the set
        wether or not they are marked as being in maze
        """
    ...
    def get_neighbors(self, x: int, y: int) -> list[Tuple[int, int, Any]]:
        """This function gets a cells all neighbors that it could go to
        ...
        Returns:
        list: all the neighbors coordinates and the direction to go there
        """
        directions = {
            'N': (0, -1),
            'S': (0, 1),
    ...
    def bfs_solver(self, start: Tuple[int, int], end: Tuple[int, int]) -> str:
        """This function is the maze solver that uses backtracking by saving
        every cell from the start and how it came into the cell and when it
        finds the exit it checks where it came from saves that into a string
        and reverses it

        Parameters:
        start (Tuple): the entrance to the maze
        end (Tuple): the exit for the maze

        Returns:
        str: A path from start to end
        """
        queue = deque([start])
        visited = set([start])
