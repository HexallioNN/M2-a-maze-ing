...
change_colours = Button("button_img/button_change-colour.png",
                        (0xEEEE0000).to_bytes(4, 'little'))
change_logo_colours = Button("button_img/button_change-logo.png",
...
def pre_render_ui_env(m: Mlx, mlx_ptr: int, dimensions: tuple[int, int, int],
    ...
    for offset in range(0, sl * y, 4):
        ...
        mod_32 = distance % 64
        if mod_16 < mod_32:
            colour = (0xFF808080).to_bytes(4, 'little')
        else:
            colour = (0xFFA0A0A0).to_bytes(4, 'little')
        if (offset / sl <= 10) or offset / sl >= empty_space_ver + 10:
            image.data[offset:offset+4] = colour
        if (offset % sl <= 40) or offset % sl >= sl - ((10 + sidebar) * 4):
            image.data[offset:offset+4] = colour
